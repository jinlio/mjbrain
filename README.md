...
- **悬浮 HUD**：`hud/float.py` 一个 OS 级置顶小窗浮在游戏窗口之上（不注入页面、不占布局），尾随推荐记录实时刷新：推荐动作 + 候选概率 + 向听/待牌/宝牌。捕获层在跑时，浮窗**跟随浏览器窗口移动**并保持相对位置（CDP 只读查询窗口几何→`~/.mjbrain/browser-bounds.json`）；右键菜单/L 键**锁定位置**防误触，位置（含相对浏览器的偏移）记住上次。`run_demo` 一键链里自带。
