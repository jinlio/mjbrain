...
[Mortal](https://github.com/herentus/mortal)、微软的 Suphx、akochan 这类是"自己下场打"的完整选手：整局牌由网络（或搜索引擎）从头打到尾，Mortal 和 Suphx 都是拿几百万局自对局的强化学习喂出来的。本项目反过来，做的是"站在旁边出主意"的辅助系统。差别不在少几个功能，是路线不同：

- **只出建议，打不了牌**。数据入口只有 CDP 只读订阅一条，代码里不存在发出牌请求的路径。Mortal 生态里常见的托管代打（比如往天凤个室放 bot）我们不做——这不是少写一个功能，是定位。
- **规则不进模型**。合法动作、向听、点数由 riichienv 确定性算出，模型只在合法子集上做带概率的选择；端到端那条路线是把这些全学进权重、靠规模兜底。
- **蒸馏，不是自对局**。教师是 Mortal，拿现成高水平牌谱做软标签蒸馏（RLCD 风格）；单卡 8GB、几小时训一轮，整条管线在一台家用机上就能复现。自对局强化学习复现不起，也不需要——目标是"够用的实时建议"，不是打上天凤位。
- **实时是一等公民**。雀魂 → CDP → liqi → mjai 的捕获管道就是为对局中实时出建议设计的，每个决策窗请求一次 `/v1/react`；牌谱复盘反而只是附赠的档 0。
- **强度上它是 Mortal 的学生**，差距开头已经说了：适合学习、改造、跑自己的实验，不适合拿去比段位。
